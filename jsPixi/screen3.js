...
//init 1
	...
	function onDown(event) {
		...
		if (f <= 1) {
			Coords[2 + f] = new Location(x, y);
			f++;
		}
		if (f == 2) {
			objs[4].addLeg(Coords[2], 2);
			objs[4].addLeg(Coords[3], 3);
		}
...
function play(){
	...
	var Look2 = function(){
		...
		objs.push(new Quarter_Back("qb", new Location(width / 2 - 20, 10 * height - 40), .2, objs[0]));
		...
		Sprites.push(PIXI.Sprite.fromImage("Pan_Blue_Circle.png"));
		Sprites.push(PIXI.Sprite.fromImage("pink_circle.png"));
		Sprites.push(PIXI.Sprite.fromImage("Pan_Blue_Circle.png"));
	...
	var but1 = PIXI.Sprite.fromImage("draw_path.png");
	...
	var set_Backs_Pass2 = function(){
		...
		Coords.push(new Location(0, 0));		
		Coords.push(new Location(0, 0));												//[5] recv3
		...
		objs[3].addLeg(Coords[1], 2);
		objs[5].addLeg(Coords[4], 3.5);

		objs[22].addLeg(Coords[5], .2);
		objs[22].addLeg(Coords[6], .7);
		objs[22].addLeg(Coords[7], 2.25);
	}
	...
	var set_Line_Pass = function(){
		...
		objs[11].addLeg(Coords[8], .6);
		objs[11].addLeg(Coords[9], 1);
		objs[12].addLeg(Coords[10], .9);
		objs[13].addLeg(Coords[11], .9);
		objs[14].addLeg(Coords[12], .9);
		objs[15].addLeg(Coords[13], .6);
		objs[15].addLeg(Coords[14], 1);

		objs[16].addLeg(Coords[15], 1.5);
		objs[17].addLeg(Coords[16], 1.2);
		objs[18].addLeg(Coords[17], 1.2);
		objs[19].addLeg(Coords[18], 1.2);
		objs[20].addLeg(Coords[19], 1.5);

		objs[21].addLeg(Coords[20], .75);
		objs[21].addLeg(Coords[21], .75);
		objs[21].addLeg(Coords[22], 2);
	}
	...
	var Disp_Routes = function(){
		...
		makeLine(recv4.loc, Coords[4], 0xffff00);
